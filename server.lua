...
ESX.RegisterServerCallback("hw_billing:GetInvoices", function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM hw_billing WHERE receiver_identifier = @identifier ORDER BY CASE WHEN status = "unpaid" THEN 1 WHEN status = "autopaid" THEN 2 WHEN status = "paid" THEN 3 WHEN status = "cancelled" THEN 4 END ASC, id DESC', {
...
RegisterServerEvent("hw_billing:PayInvoice")
AddEventHandler("hw_billing:PayInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM hw_billing WHERE id = @id', {
		['@id'] = invoice_id
	}, function(result)
		...
		if playerMoney < invoices.invoice_value then
			...
			MySQL.Async.execute('UPDATE hw_billing SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})
...
RegisterServerEvent("hw_billing:CancelInvoice")
AddEventHandler("hw_billing:CancelInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM hw_billing WHERE id = @id', {
		...
		MySQL.Async.execute('UPDATE hw_billing SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
...
RegisterServerEvent("hw_billing:CreateInvoice")
AddEventHandler("hw_billing:CreateInvoice", function(data)
	local _source = ESX.GetPlayerFromId(source)
	local target = ESX.GetPlayerFromId(data.target)
	local webhookData = {}

	MySQL.Async.fetchAll('SELECT id FROM hw_billing WHERE id = (SELECT MAX(id) FROM hw_billing)', {}, function(result)
		webhookData = {
			id = result[1].id + 1,
...
ESX.RegisterServerCallback("hw_billing:GetSocietyInvoices", function(source, cb, society)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM hw_billing WHERE society_name = @society ORDER BY id DESC', {
		['@society'] = society
	}, function(result)
		local invoices = {}
...
function checkTimeLeft()
	MySQL.Async.fetchAll('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM hw_billing WHERE status = @status', {
		['@status'] = 'unpaid'
	}, function(result)
				...
				for k, vl in pairs(whenToAddFees) do
					if v.fees_amount == k - 1 then
						if v.timeLeft >= vl*(-1) then
							MySQL.Async.execute('UPDATE hw_billing SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
								['@fees_amount'] = k,
						...
						MySQL.Async.execute('UPDATE users SET accounts = @playerAccount WHERE identifier = @target', {
							...
							TriggerEvent("esx_addonaccount:getSharedAccount", v.society, function(account2)
								if account2 ~= nil then
									...
									MySQL.Async.execute('UPDATE hw_billing SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
										['@paid'] = 'autopaid',
										['@id'] = v.id
									})
								end
